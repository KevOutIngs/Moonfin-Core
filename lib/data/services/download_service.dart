...
class DownloadService extends ChangeNotifier implements AutoDownloadDownloader {
  ...
  Future<int?> _deviceFreeBytes() async {
    try {
      ...
      debugPrint('[Downloads] free space at ${root.path}: $free');
      return free;
    } catch (e) {
      debugPrint('[Downloads] free space unknown: $e');
      return null;
    }
  }
  ...
  Future<int?> storageHeadroomBytes() async {
    ...
    debugPrint(
      '[Downloads] headroom: limit=${limitMb}MB free=$free '
  ...
  Future<String?> _storageRefusal(int estimatedBytes) async {
    ...
    final free = await _deviceFreeBytes();
    debugPrint('[Downloads] admit ${formatBytes(estimatedBytes)}? free=$free');
    if (free != null && estimatedBytes + _reservedBytes + _freeSpaceMargin > free) {
  ...
  Future<Set<String>> _refuseKilledTasksThatCannotFit(
    ...
    for (final entry in records.entries) {
      ...
      unawaited(_notificationService.showError(itemName: label, error: refusal));
