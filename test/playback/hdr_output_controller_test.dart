...
void main() {
  ...
  group('HdrOutputStatus', () {
    test('reports HDR10 only while active', () {
      const active = HdrOutputStatus(HdrOutputReason.active);
      expect(active.isActive, isTrue);
      expect(active.output, contains('HDR10'));

      for (final reason in HdrOutputReason.values.where(
        (r) => r != HdrOutputReason.active,
      )) {
        expect(HdrOutputStatus(reason).output, 'SDR');
      }
    });
  });
  ...
  group('HdrOutputController.maybeEngage', () {
    ...
    test('every gate open engages and hands mpv the handle', () async {
      ...
      expect(controller.status.value.reason, HdrOutputReason.active);
    ...
    test('mpv refusing the handle fails and tears the window down', () async {
      ...
      expect(controller.status.value.reason, HdrOutputReason.failed);
  ...
  group('HdrOverlayChannel', () {
    ...
    test('a missing plugin latches off, so a non-Windows build goes quiet', () async {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(channel, (call) async {
            calls.add(call);
