...
class AutoDownloadService extends ChangeNotifier {
  AutoDownloadService({
    ...
    _lastRun = AutoDownloadRunSummary.fromJson(
      prefs.get(UserPreferences.autoDownloadLastRun),
    );
  }
  ...
  static bool runsForegroundOnly(DownloadQuality quality) =>
  ...
  Stream<List<AutoDownloadSubscription>> watchSubscriptions() =>
      repository.watchSubscriptions(serverId: serverId, userId: userId);

  Stream<AutoDownloadSubscription?> watchSubscription(String seriesId) =>
      repository.watchSubscription(
        seriesId: seriesId,
        serverId: serverId,
        userId: userId,
      );
  ...
  Future<void> unsubscribe(String seriesId) => repository.deleteSubscription(
    seriesId: seriesId,
    serverId: serverId,
    userId: userId,
  );
  ...
  Future<AutoDownloadRunSummary> _check(
    ...
    if (!prefs.get(UserPreferences.autoDownloadEnabled)) {
    ...
    await ready;
    final state = await _SharedState.load(repository, downloader);
    final playing = playingItemId?.call();
    final keepSetting = prefs.get(UserPreferences.autoDownloadKeepUnwatched);
